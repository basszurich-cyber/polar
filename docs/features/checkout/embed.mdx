...
You can style the trigger element any way you want, as long as you keep the `data-spaire-checkout` attribute.
...
Then, you should import the `SpaireEmbedCheckout` helper class and manually call `SpaireEmbedCheckout.init()`. This will add the required handlers on elements having the `data-spaire-checkout` attribute.
...
For users who need more control over the embedded checkout flow, the `SpaireEmbedCheckout` class provides several advanced features.

### Programmatically creating an embed

Instead of using declarative triggers with `data-spaire-checkout` attributes, you can programmatically create and control checkout instances:
...
const openCheckout = async () => {
  ...
  try {
    ...
    const checkout = await SpaireEmbedCheckout.create(checkoutLink, theme);
...
const openCheckoutWithEvents = async () => {
  const checkout = await SpaireEmbedCheckout.create("__CHECKOUT_LINK__");
...
const CheckoutButton = () => {
  ...
  const handleCheckout = async () => {
      try {
        const checkout = await SpaireEmbedCheckout.create(
          '__CHECKOUT_LINK__',
          'light'
        )
