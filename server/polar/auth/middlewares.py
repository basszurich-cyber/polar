...
def get_bearer_token_from_websocket(scope: ASGIScope) -> str | None:
    ...
    scheme, value = get_authorization_scheme_param(authorization)
    if scheme and value and scheme.lower() == "bearer" and value.isascii():
        return value
...
async def get_auth_subject_from_websocket(
    scope: ASGIScope, session: AsyncSession
) -> AuthSubject[Subject]:
    """Get auth subject from WebSocket connection."""
    token = get_bearer_token_from_websocket(scope)
    if token is not None:
        if is_registration_token_prefix(token):
            return AuthSubject(Anonymous(), set(), None)
    ...
    # WebSockets don't support user sessions (cookies), so return Anonymous
    return AuthSubject(Anonymous(), set(), None)
    ...
    async def __call__(self, scope: ASGIScope, receive: Receive, send: Send) -> None:
        if scope["type"] == "http":
            ...
            try:
                ...
                # For WebSocket, we can't return an HTTP response
                # The error will be handled when the connection is accepted
                log.warning("WebSocket authentication failed", error=str(e))
                auth_subject = AuthSubject(Anonymous(), set(), None)

            scope["state"]["auth_subject"] = auth_subject

            with logfire.set_baggage(**auth_subject.log_context):
                log.info("Authenticated WebSocket subject", **auth_subject.log_context)
                set_sentry_user(auth_subject)
                await self.app(scope, receive, send)
